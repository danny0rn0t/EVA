...
import os
import sys
from dataset_small import MOSEIDataset
config = {
    "num_label": 7,
    "train_batch_size": 1,
    "logging_step": 1000,
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.hubert_layers = HubertModel.from_pretrained(
        ...
        self.linear = nn.Sequential(
                ...
                nn.Linear(4096, 4096),
                nn.ReLU(),
                nn.Linear(4096, config["num_label"]))
    def forward(self, x):
        x=self.hubert_layers(input_values=x).last_hidden_state[:,0]
...
for name, param in model.hubert_layers.named_parameters():
    # print(name)
